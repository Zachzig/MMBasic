...
Do  
	...
	Do
	  ...
	  If player = 1 Or gameMode = 1 Then
		...
		Print @(OFFSET_X-12, 5) "Player " + Str$(player) + ": select column (0-6)"
		Do
		  column = Asc(Inkey$) - 48
		Loop Until column >= 0 And column <= 6
	  Else
		Print @(OFFSET_X-12, 5) "Computer thinks...          "
		Pause 600
	  ...
	  If row <> -1 Then
		...
		If win = 1 Then
		  ...
		  Text MM.HRES/2, 10, "Winner: Player " + Str$(player), "CM", 1, 2, RGB(YELLOW)
		  Pause 3000
		  Exit Do
		ElseIf moves = 42 Then
		  GoSub DrawBoard
		  Box 0, 0, MM.HRES,15, 1, RGB(BLACK), RGB(BLACK)
		  Text MM.HRES/2, 10, "No Winner!", "CM", 1, 2, RGB(WHITE)
		  Pause 3000
		  Exit Do
		End If
		player = 3 - player
...
ShowMenu:
  ...
  Text MM.HRES/2, 50, "Connect Four", "CM", 1, 3, RGB(CYAN)
  Text MM.HRES/2, 100, "1: Human vs Human", "CM", 1, 1, RGB(WHITE)
  Text MM.HRES/2, 130, "2: Human vs Computer", "CM", 1, 1, RGB(WHITE)
