...
' 01/2026 P.Doerwald and Gemini  V1.0
' ------------------------------------
Option Explicit
...
Do  
	...
	Do
	  ...
	  If player = 1 Or gameMode = 1 Then
		' Eingabe C<ber Tasten 0-6
		Print @(0, 5) "Spieler " + Str$(player) + ": Waehle Spalte (0-6)"
		Do
		  column = Asc(Inkey$) - 48
		Loop Until column >= 0 And column <= 6
	  Else
		Print @(0, 5) "Computer denkt nach...          "
		Pause 600
...
ShowMenu:
  ...
  For i = 0 To 80
    ...
    Circle i-2, 160, 12, 0, 1, RGB(BLACK), RGB(BLACK)
    Circle 240-i+2, 160, 12, 0, 1, RGB(BLACK), RGB(BLACK)
    ' Neuer Zustand zeichnen
    Circle i, 160, 12, 1, 1, RGB(YELLOW), RGB(YELLOW) ' Spieler 1 rollt von links
    Circle 240-i, 160, 12, 1, 1, RGB(RED), RGB(RED)    ' Spieler 2 rollt von rechts
    Pause 15
  Next i
  Text 120, 50, "4 GEWINNT", "CM", 1, 3, RGB(CYAN)
  Text 120, 100, "1: Mensch vs Mensch", "CM", 1, 1, RGB(WHITE)
  Text 120, 130, "2: Mensch vs Computer", "CM", 1, 1, RGB(WHITE)
  Do
